 ...
 *
 * Sidebar izquierdo que se expande al acercar el cursor.
 * - Colapsado: solo iconos (56px ancho)
 * - Expandido: iconos + labels (200px ancho)
 * - Mismo pipeline de vidrio que MagnifyingGlass
 * - Portalado a document.body
 */
...
const defaultItems: LeftSidebarItem[] = [
  {
...
export const LiquidGlassLeftSidebar: React.FC<{ items?: LeftSidebarItem[] }> = ({
  items = defaultItems,
}) => {
  const rawId = useId();
  const filterId = `lg-left-filter-${rawId.replace(/[^a-zA-Z0-9]/g, '')}`;
  ...
  // ── Texture maps — generados para el tamaño expandido ─────────────────────
  const MAP_W = Math.min(512, Math.max(128, Math.round(EXPANDED_W * 1.5)));
  const MAP_H = Math.min(512, Math.max(128, Math.round(HEIGHT     * 1.5)));
  const mapBR = (24 / Math.min(EXPANDED_W, HEIGHT)) * Math.min(MAP_W, MAP_H);
  ...
  const { zoomMapUrl, rimMapUrl, zoomScale, rimScale, padPercent } = useMemo(() => {
    ...
    const rimResult = generateDisplacementMap({
      ...
      bezelPixelWidth: (bezelThickness / Math.min(EXPANDED_W, HEIGHT)) * Math.min(MAP_W, MAP_H),
    ...
    const bboxSpan = (EXPANDED_W + HEIGHT) / 2;
    ...
    <>
      ...
      <div
        ...
        style={{
          ...
          overflow:    'hidden', // necesario para que el texto no desborde al colapsar
        }}
      >
        {/* Capa de vidrio */}
        <div style={{
        ...
        <div style={{
          ...
          justifyContent: 'center', gap: 4, padding: '20px 0',
          // ancho fijo para que al animar width el contenido no se comprima
          width: EXPANDED_W,
        }}>
          {items.map((item, idx) => (
            <button
